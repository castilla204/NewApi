...

    {
...

        {
...

            {
                ...
                // ✅ CORREGIDO: Eliminado EntityState.Modified - EF Core detecta cambios automáticamente
                // El archivo antiguo funcionaba sin forzar el estado manualmente
                // Forzar EntityState.Modified causaba conflictos en K8s con la configuración de tracking
                ...
                // ✅ LOG: Verificar timers "proposal" que se van a cancelar
                await _loggingService.LogInfoAsync(
                    message: "Cancelando timers 'proposal' antes de crear timer 'response'",
                    details: $"AppointmentId: {appointment.Id}, Timers 'proposal' encontrados: {proposalTimers.Count}, " +
                            $"JobIds a cancelar: [{string.Join(", ", hangfireJobIdsToCancel)}]",
                    userId: userId,
                    source: "AppointmentService.ProposeAppointmentAsync",
                    relatedEntityType: "Appointment",
                    relatedEntityId: appointment.Id,
                ...
                // ✅ LOG CRÍTICO: Verificar qué se guardó
                await _loggingService.LogCriticalAsync(
                    message: "CRITICAL: SaveChanges ejecutado en ProposeAppointment - Verificación de timers",
                    details: $"SaveChangesResult: {saveChangesResult} entidades modificadas. " +
                            $"AppointmentId: {appointment.Id}, " +
                            $"Timers 'proposal' marcados como expirados: {proposalTimers.Count}, " +
                            $"Timer 'response' creado: Id={responseTimer.Id}, " +
                            $"Verificar que los timers 'proposal' se guardaron como expirados",
                    userId: userId,
                    source: "AppointmentService.ProposeAppointmentAsync",
                    relatedEntityType: "Appointment",
                    relatedEntityId: appointment.Id,
                    additionalData: new
                    {
                        ...
                        ProposalTimersCount = proposalTimers.Count,
                        ResponseTimerId = responseTimer.Id,
                        HangfireJobIdsToCancel = hangfireJobIdsToCancel
                    }
                );
                ...
                await _loggingService.LogInfoAsync(
                    ...
                    notifyUser: false
                        ...
                        // ✅ CORREGIDO: Eliminado EntityState.Detached - no es necesario forzar recarga
                        // El archivo antiguo funcionaba sin esto, y causaba problemas en K8s
                        ...
                        // ✅ CRÍTICO: Recargar el timer desde la BD para asegurar que está trackeado correctamente
                        var responseTimerToUpdate = await _context.AppointmentTimers
                            .FirstOrDefaultAsync(t => t.Id == responseTimer.Id);
                        
                        if (responseTimerToUpdate != null)
                        {
                            responseTimerToUpdate.HangfireJobId = responseJobId;
                            // ✅ CORREGIDO: Eliminado EntityState.Modified - EF Core detecta cambios automáticamente
                            await _context.SaveChangesAsync();
                        }
                        else
                        {
                            // ✅ LOG: Si no se encuentra el timer, loguear error crítico
                            await _loggingService.LogCriticalAsync(
                                message: "CRITICAL: Timer de respuesta no encontrado para actualizar HangfireJobId",
                                details: $"TimerId: {responseTimer.Id}, AppointmentId: {appointment.Id}, ResponseJobId: {responseJobId}",
                                userId: userId,
                                source: "AppointmentService.ProposeAppointmentAsync",
                                relatedEntityType: "AppointmentTimer",
                                relatedEntityId: responseTimer.Id
                            );
                        }

                // ✅ CORREGIDO: Eliminado AsNoTracking - no es necesario después de eliminar Detach
                // El archivo antiguo funcionaba sin esto
...

        {
...

            {
                ...
                // ✅ CRÍTICO: Marcar timers de respuesta como expirados y cancelar jobs de Hangfire
                var responseTimers = await _context.AppointmentTimers
                    .Where(t => t.AppointmentId == appointment.Id && 
                               t.TimerType == "response" && 
                               !t.IsExpired)
                ...
                // ✅ CRÍTICO: Cancelar timers "proposal" existentes antes de crear uno nuevo
                // Esto evita duplicación de timers y jobs de Hangfire
                var existingProposalTimers = await _context.AppointmentTimers
                    .Where(t => t.AppointmentId == appointment.Id && 
                               t.TimerType == "proposal" && 
                               !t.IsExpired)
                    .ToListAsync();

                foreach (var timer in existingProposalTimers)
                {
                    timer.IsExpired = true;
                    timer.ExpiredAt = DateTime.UtcNow;
            ...
            try
            {
                ...
                switch (timer.TimerType)
                {
                    case "proposal":
                        try
                        {
                            // ✅ LOG CRÍTICO: Inicio del procesamiento del timer "proposal"
                            await _loggingService.LogCriticalAsync(
                                message: "CRITICAL: Iniciando procesamiento de timer 'proposal'",
                                details: $"TimerId: {timerId}, AppointmentId: {timer.AppointmentId}, SearchHireId: {timer.Appointment?.SearchHireId}, " +
                                        $"AppointmentStatus actual: '{appointmentStatus}', SearchHireStatus actual: '{searchHireStatus}', " +
                                        $"Timer.IsExpired: {timer.IsExpired}, Timer.EndTime: {timer.EndTime}, Now: {DateTime.UtcNow}",
                                userId: timer.Appointment?.SearchHire?.ClientId,
                                source: "AppointmentService.ProcessAppointmentTimerAsync.proposal",
                                relatedEntityType: "AppointmentTimer",
                                relatedEntityId: timerId,
                                additionalData: new
                                {
                                    ...
                                    TimerIsExpired = timer.IsExpired,
                            ...
                            // ✅ LOG CRÍTICO: Verificar si el estado existe
                            if (noProposalStatus == null)
                            {
                                await _loggingService.LogCriticalAsync(
                                    message: "CRITICAL: Estado 'appointment_cancelled_by_client_no_proposal' NO ENCONTRADO en SystemStatuses",
                                    details: $"Timer {timerId} no puede procesarse porque el estado 'appointment_cancelled_by_client_no_proposal' no existe en la base de datos. " +
                                            $"AppointmentId: {timer.AppointmentId}, SearchHireId: {timer.Appointment?.SearchHireId}. " +
                                            $"Esto impide que el timer procese la cancelación automática.",
                                    userId: timer.Appointment?.SearchHire?.ClientId,
                                    source: "AppointmentService.ProcessAppointmentTimerAsync.proposal",
                                    relatedEntityType: "AppointmentTimer",
                                    relatedEntityId: timerId,
                                    additionalData: new
                                    {
                                        ...
                                        AppointmentId = timer.AppointmentId,
                                        SearchHireId = timer.Appointment?.SearchHireId,
                                        ExpectedStatusValue = AppointmentStatus.AppointmentCancelledByClientNoProposal.ToStringValue()
                                    }
                                );
                                break; // No continuar si el estado no existe
                            }

                            // ✅ LOG CRÍTICO: Verificar que Appointment existe
                            if (timer.Appointment == null)
                            {
                                await _loggingService.LogCriticalAsync(
                                    message: "CRITICAL: Appointment es NULL en timer 'proposal'",
                                    details: $"Timer {timerId} tiene Appointment null. No se puede procesar la cancelación.",
                                    userId: null,
                                    source: "AppointmentService.ProcessAppointmentTimerAsync.proposal",
                                    relatedEntityType: "AppointmentTimer",
                                    relatedEntityId: timerId
                            ...
                            // ✅ LOG CRÍTICO: Antes de cambiar el estado
                            await _loggingService.LogCriticalAsync(
                                message: "CRITICAL: Cambiando estado de Appointment a 'appointment_cancelled_by_client_no_proposal'",
                                details: $"TimerId: {timerId}, AppointmentId: {timer.AppointmentId}, " +
                                        $"Estado actual Appointment.StatusId: {timer.Appointment.StatusId}, " +
                                        $"Nuevo estado StatusId: {noProposalStatus.Id}, " +
                                        $"StatusValue: {noProposalStatus.StatusValue}",
                                userId: timer.Appointment.SearchHire?.ClientId,
                                source: "AppointmentService.ProcessAppointmentTimerAsync.proposal",
                                relatedEntityType: "Appointment",
                                relatedEntityId: timer.Appointment.Id,
                                additionalData: new
                                {
                                    TimerId = timerId,
                                    AppointmentId = timer.AppointmentId,
                                    OldStatusId = timer.Appointment.StatusId,
                                    NewStatusId = noProposalStatus.Id,
                                    NewStatusValue = noProposalStatus.StatusValue
                                }
                            );
                            ...
                            var appointmentSaveChangesResult = await _context.SaveChangesAsync();

                            // ✅ LOG CRÍTICO: Verificar que SaveChanges funcionó
                            await _loggingService.LogCriticalAsync(
                                message: "CRITICAL: SaveChanges ejecutado después de cambiar estado de Appointment",
                                details: $"TimerId: {timerId}, AppointmentId: {timer.AppointmentId}, " +
                                        $"SaveChangesResult: {appointmentSaveChangesResult} entidades modificadas, " +
                                        $"Appointment.StatusId después de SaveChanges: {timer.Appointment.StatusId}, " +
                                        $"Appointment.UpdatedAt: {timer.Appointment.UpdatedAt}",
                                userId: timer.Appointment.SearchHire?.ClientId,
                                source: "AppointmentService.ProcessAppointmentTimerAsync.proposal",
                                relatedEntityType: "Appointment",
                                relatedEntityId: timer.Appointment.Id,
                                additionalData: new
                                {
                                    TimerId = timerId,
                                    AppointmentId = timer.AppointmentId,
                                    SaveChangesResult = appointmentSaveChangesResult,
                                    AppointmentStatusId = timer.Appointment.StatusId,
                                    AppointmentUpdatedAt = timer.Appointment.UpdatedAt
                                }
                            );
                            ...
                            try
                            {
                                // ✅ LOG CRÍTICO: Antes de llamar a ProcessMoneyDistributionAsync
                                await _loggingService.LogCriticalAsync(
                                    message: "CRITICAL: Llamando a ProcessMoneyDistributionAsync para timer 'proposal'",
                                    details: $"TimerId: {timerId}, AppointmentId: {timer.AppointmentId}, SearchHireId: {timer.Appointment.SearchHireId}, " +
                                            $"StatusValue: {AppointmentStatus.AppointmentCancelledByClientNoProposal.ToStringValue()}, " +
                                            $"updateState: true (debe actualizar SearchHire automáticamente)",
                                    userId: timer.Appointment.SearchHire?.ClientId,
                                    source: "AppointmentService.ProcessAppointmentTimerAsync.proposal",
                                    relatedEntityType: "Appointment",
                                    relatedEntityId: timer.Appointment.Id,
                                ...
                                // ✅ LOG CRÍTICO: Después de ProcessMoneyDistributionAsync
                                // Recargar el SearchHire para verificar que se actualizó
                                // ✅ FIX: Usar AsNoTracking para evitar conflictos de tracking
                                string searchHireStatusAfterMoney = "null";
                                int? searchHireStatusIdAfterMoney = null;
                                
                                if (timer.Appointment.SearchHire != null)
                                {
                                    var searchHireId = timer.Appointment.SearchHire.Id;
                                    
                                    // Recargar desde la base de datos con AsNoTracking para solo leer valores
                                    var reloadedSearchHire = await _context.SearchHires
                                        .AsNoTracking()
                                        .Include(sh => sh.Status)
                                        .FirstOrDefaultAsync(sh => sh.Id == searchHireId);
                                    
                                    if (reloadedSearchHire != null)
                                    {
                                        searchHireStatusAfterMoney = reloadedSearchHire.Status?.StatusValue ?? "null";
                                        searchHireStatusIdAfterMoney = reloadedSearchHire.StatusId;
                                        // ✅ CRÍTICO: NO asignar la entidad recargada directamente para evitar conflictos de tracking
                                        // Solo usar los valores para logging
                                    }
                                }

                                await _loggingService.LogCriticalAsync(
                                    message: "CRITICAL: ProcessMoneyDistributionAsync completado para timer 'proposal'",
                                    details: $"TimerId: {timerId}, AppointmentId: {timer.AppointmentId}, SearchHireId: {timer.Appointment.SearchHireId}, " +
                                            $"SearchHireStatus después de ProcessMoneyDistributionAsync: '{searchHireStatusAfterMoney}', " +
                                            $"SearchHire.StatusId: {searchHireStatusIdAfterMoney ?? 0}",
                                    userId: timer.Appointment.SearchHire?.ClientId,
                                    source: "AppointmentService.ProcessAppointmentTimerAsync.proposal",
                                    relatedEntityType: "Appointment",
                                    relatedEntityId: timer.Appointment.Id,
                                    additionalData: new
                                    {
                                        ...
                                        AppointmentId = timer.Appointment.Id,
                                        SearchHireId = timer.Appointment.SearchHireId,
                                        SearchHireStatusAfterMoney = searchHireStatusAfterMoney,
                                        SearchHireStatusIdAfterMoney = searchHireStatusIdAfterMoney
                                    }
                                );

                                // ✅ CRÍTICO: NO hacer SaveChangesAsync aquí porque ProcessMoneyDistributionAsync ya hizo commit de su transacción
                                // El SearchHire ya fue actualizado y commiteado en RefundService.ProcessMoneyDistributionAsync
                                // Hacer SaveChangesAsync aquí causaría conflictos de tracking porque recargamos con AsNoTracking
                                
                                await _loggingService.LogCriticalAsync(
                                    message: "CRITICAL: Verificación final después de ProcessMoneyDistributionAsync",
                                    details: $"TimerId: {timerId}, AppointmentId: {timer.AppointmentId}, " +
                                            $"ProcessMoneyDistributionAsync ya hizo commit de los cambios. " +
                                            $"SearchHireStatus verificado: '{searchHireStatusAfterMoney}', " +
                                            $"SearchHireStatusId verificado: {searchHireStatusIdAfterMoney ?? 0}. " +
                                            $"NO se ejecuta SaveChangesAsync aquí para evitar conflictos de tracking.",
                                    userId: timer.Appointment.SearchHire?.ClientId,
                                    source: "AppointmentService.ProcessAppointmentTimerAsync.proposal",
                                    relatedEntityType: "Appointment",
                                    relatedEntityId: timer.Appointment.Id,
                                    additionalData: new
                                    {
                                        TimerId = timerId,
                                        AppointmentId = timer.Appointment.Id,
                                        SearchHireStatusVerified = searchHireStatusAfterMoney,
                                        SearchHireStatusIdVerified = searchHireStatusIdAfterMoney,
                                        SaveChangesSkipped = true,
                                        Reason = "ProcessMoneyDistributionAsync already committed changes"
                                    }
                                );
                                ...
                                {
                                    await _loggingService.LogCriticalAsync(
                                        message: "CRITICAL: Error al enviar emails de notificación en timer 'proposal'",
                                        details: $"TimerId: {timerId}, AppointmentId: {timer.AppointmentId}. Error: {emailEx.Message}",
                                        userId: timer.Appointment.SearchHire?.ClientId,
                                        source: "AppointmentService.ProcessAppointmentTimerAsync.proposal",
                                        relatedEntityType: "Appointment",
                                        relatedEntityId: timer.Appointment.Id,
                            ...
                            {
                                // ✅ LOG CRÍTICO: El dinero falló pero el estado ya está guardado
                                await _loggingService.LogCriticalAsync(
                                    message: "CRITICAL: Money distribution failed after auto-cancel (proposal timer)",
                                    details: $"Timer {timerId}, AppointmentId {timer.AppointmentId}, SearchHireId {timer.Appointment.SearchHireId}. " +
                                             $"State is already updated to appointment_cancelled_by_client_no_proposal. " +
                                             $"Exception Type: {moneyEx.GetType().Name}, Exception Message: {moneyEx.Message}, " +
                                             $"Inner Exception: {moneyEx.InnerException?.Message ?? "None"}, " +
                                             $"Stack Trace: {moneyEx.StackTrace}",
                                    userId: timer.Appointment.SearchHire?.ClientId,
                                    source: "AppointmentService.ProcessAppointmentTimerAsync.proposal",
                                    relatedEntityType: "Appointment",
                                    relatedEntityId: timer.Appointment.Id,
                                    additionalData: new
                                    {
                                        ...
                                        ExceptionType = moneyEx.GetType().Name,
                                        ExceptionMessage = moneyEx.Message,
                                        InnerException = moneyEx.InnerException?.Message,
                            ...
                            // ✅ LOG CRÍTICO: Finalización del procesamiento del timer "proposal"
                            await _loggingService.LogCriticalAsync(
                                message: "CRITICAL: Procesamiento de timer 'proposal' COMPLETADO",
                                details: $"TimerId: {timerId}, AppointmentId: {timer.AppointmentId}, SearchHireId: {timer.Appointment.SearchHireId}, " +
                                        $"Appointment.StatusId final: {timer.Appointment.StatusId}, " +
                                        $"SearchHire.StatusId final: {timer.Appointment.SearchHire?.StatusId}",
                                userId: timer.Appointment.SearchHire?.ClientId,
                                source: "AppointmentService.ProcessAppointmentTimerAsync.proposal",
                                relatedEntityType: "Appointment",
                                relatedEntityId: timer.Appointment.Id,
                                additionalData: new
                                {
                                    ...
                                    AppointmentId = timer.Appointment.Id,
                                    SearchHireId = timer.Appointment.SearchHireId,
                                    FinalAppointmentStatusId = timer.Appointment.StatusId,
                                    FinalSearchHireStatusId = timer.Appointment.SearchHire?.StatusId
                                }
                            );
                        }
                        catch (Exception proposalEx)
                        {
                            // ✅ LOG CRÍTICO: Error general en el procesamiento del timer "proposal"
                            await _loggingService.LogCriticalAsync(
                                message: "CRITICAL: Error general en procesamiento de timer 'proposal'",
                                details: $"TimerId: {timerId}, AppointmentId: {timer.AppointmentId}, SearchHireId: {timer.Appointment?.SearchHireId}. " +
                                        $"Exception Type: {proposalEx.GetType().Name}, Exception Message: {proposalEx.Message}, " +
                                        $"Inner Exception: {proposalEx.InnerException?.Message ?? "None"}, " +
                                        $"Stack Trace: {proposalEx.StackTrace}",
                                userId: timer.Appointment?.SearchHire?.ClientId,
                                source: "AppointmentService.ProcessAppointmentTimerAsync.proposal",
                                relatedEntityType: "AppointmentTimer",
                                relatedEntityId: timerId,
                                additionalData: new
                                {
                                    ...
                                    AppointmentId = timer.AppointmentId,
                                    SearchHireId = timer.Appointment?.SearchHireId,
                                    ExceptionType = proposalEx.GetType().Name,
                                    ExceptionMessage = proposalEx.Message,
                                    InnerException = proposalEx.InnerException?.Message,
                                    StackTrace = proposalEx.StackTrace
                                },
                                notifyUser: true
                            );
                            throw; // Re-lanzar para que Hangfire lo reintente
                        }
                        break;
                ...
                // ✅ CRÍTICO: Asegurar que el timer esté marcado como modificado antes de SaveChanges
                // El timer se marca como expirado al inicio (línea 5002-5003), pero EF Core puede no detectarlo
                if (!timer.IsExpired)
                {
                    timer.IsExpired = true;
                    timer.ExpiredAt = DateTime.UtcNow;
                ...
                await _loggingService.LogInfoAsync(
                    ...
                    details: $"Timer {timerId} procesado. Guardando cambios: IsExpired={timer.IsExpired}, ExpiredAt={timer.ExpiredAt}, AppointmentStatusId={timer.Appointment?.StatusId}, " +
                            $"AppointmentId={timer.AppointmentId}",
                    userId: timer.Appointment?.SearchHire?.ClientId,
                    source: "AppointmentService.ProcessAppointmentTimerAsync",
                    relatedEntityType: "AppointmentTimer",
                ...
                var finalSaveChangesResult = await _context.SaveChangesAsync();
                ...
                await _loggingService.LogInfoAsync(
                    ...
                    details: $"Timer {timerId} procesado y cambios guardados en base de datos. " +
                            $"FinalSaveChangesResult: {finalSaveChangesResult} entidades modificadas, " +
                            $"Timer.IsExpired: {timer.IsExpired}, Timer.ExpiredAt: {timer.ExpiredAt}, " +
